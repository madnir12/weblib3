...
// this function use to check if obj has contains an value or not
export const objHas = (obj, searchStr)=>{
  for (var key in obj) {
    if (obj.hasOwnProperty(key) && typeof obj[key] === "string") {
      if (obj[key].includes(searchStr)) {
        return true; // Found a property containing the search string
      }
    }
  }
  return false; // No property containing the search string found
}
...
export function wrapText(className) {
  ...
  if (parent.innerText === selection.toString()) {
    if (
      parent.id !== "myTa" &&
      !parent.classList.contains("page-div-container")
    ) {
