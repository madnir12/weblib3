...
const AllMatch = ({ item, query }) => {
  ...
  const totalFounds = item.pages.filter((item) =>
    getText(item.content).includes(query)
  ).length;
  ...
  return (
    <div className="single-match-container">
      <div className="book-row" onClick={() => setPagesDisplay(!pagesDisplay)}>
        ...
        <div className="details">
          ...
          <h4 className="text-secondary">
            Created At: {timeSince(bookCreatedAt)}
          </h4>
      ...
      <div
        className={
          !pagesDisplay ? "pages-container show hide" : "pages-container"
        }
      >
